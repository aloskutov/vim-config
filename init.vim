...
if has("win32")
    set backupdir=~/AppData/Local/nvim/backup//   " Указываем каталог для backup'ов
    set directory=~/AppData/Local/nvim/swap//     " Указываем каталог для swap файла
    set udir==~/AppData/Local/nvim/undo//         " Указываем каталог для undo файлов
endif
...
command! -nargs=* CleanHTML call CleanHTML()
function! CleanHTMLl()
    let s:count = line("$")
...
"Enable file type detection
    filetype on
    autocmd FileType php setlocal ts=4 sts=4 sw=4 et ff=unix
    ...
    autocmd FileType git setlocal ts=4 sts=4 sw=4 et
